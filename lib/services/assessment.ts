...
function publicQuestion(question: Question) {
  const { correct_answer, explanation, roadmap_reference, weight, ...safe } = question;
  return safe;
}
...
export async function startAttempt(): Promise<Result<{
  ...
  const { data: active } = await supabase
    ...
    .select("id,seed,question_ids,answers")
    .eq("user_id", user.id)
    .eq("role", role)
  ...
  if (!row) {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        seed,
        question_ids: selected(role, seed),
        status: "in_progress",
      })
      .select("id,seed,question_ids,answers")
      .single();
  ...
  const lookup = new Map(bank(role).questions.map(q => [q.id, q]));
  return {
    data: {
      attemptId: row.id,
      questions: (row.question_ids as string[]).map(id => publicQuestion(lookup.get(id)!)),
...
export async function recordAnswer(
  ...
  const { data: row } = await supabase
    .from("assessment_results")
    .select("id,role,status,question_ids,answers,answer_results")
    .eq("id", attemptId)
    .eq("user_id", user.id)
    .maybeSingle();
  ...
  results[questionId] = { is_correct: selectedIndex === question.correct_answer };
...
export async function finishAttempt(attemptId: string): Promise<Result<{
  ...
  const skillScores = bank(row.role).assessment.skills_assessed.map(skill => {
    ...
    const earned = own.reduce((n, q) => n + (row.answer_results[q.id]?.is_correct ? (q.difficulty === "Medium" ? 1.3 : 1) : 0), 0);
