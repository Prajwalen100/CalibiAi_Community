...
function StatItem({ end, suffix, label, desc, icon: Icon, glowClass, iconColor }: StatItemProps) {
  ...
  return (
    <div className="flex items-center gap-4 px-6 py-4 sm:px-8 sm:py-5">
      <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl ${iconColor} ${glowClass} border border-white/10 transition-transform duration-300 hover:scale-110`}>
        <Icon className="h-6 w-6" />
      </div>
      <div>
        <div className="flex items-baseline gap-1.5">
          <span ref={ref} className="text-2xl font-black text-white sm:text-3xl">
            {display}
          </span>
  ...
  return (
    <section className="relative py-12 sm:py-16">
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        <div
          className="glass flex flex-col items-center justify-center divide-y divide-white/8 sm:flex-row sm:divide-x sm:divide-y-0 sm:rounded-2xl overflow-hidden"
        >
          <StatItem
            end={10}
            suffix="+"
